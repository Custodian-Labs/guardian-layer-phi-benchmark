...
\title{\bfseries Surrogate Substitution Preserves PHI Detectability:\\ A Multi-Detector Equivalence Study\thanks{Code, data, and an interactive dashboard: \url{https://custodianai.pages.dev}. (Author-identifying; to be omitted in any anonymous-review version.)}}
...
\textbf{Detectability is statistically unchanged.} Restricting to the 57{,}112 masked spans and pooling all 11 detectors (via the released \tp{analyze\_equivalence.py}), recall moves $76.1\%\rightarrow74.9\%$ ($-1.2$ pts; 95\% CI $[-1.5,-1.0]$). The TOST equivalence test ($\Delta=2$) rejects non-equivalence at $p\approx3\times10^{-9}$: the change is provably bounded within $\pm2$ points of zero. Ranking is preserved; the flagship Llama~3.3-70B is essentially unchanged ($\Delta$F1~$+0.003$). The same contingency is ``significant'' under McNemar's test---a direct demonstration of the large-$N$ trap.
...
\textbf{The lost population.} Pooled across detectors and masked spans: 40{,}165 (span$\times$detector) pairs found in both conditions; 3{,}300 lost. Critically, 50\% of lost spans are the \emph{same length} as the original---not a boundary effect. By type: LOCATION 27\%, NAME 23\%, DATE/AGE 22\%, ID/contact 19\%.
