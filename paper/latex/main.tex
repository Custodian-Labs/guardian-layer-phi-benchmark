...
\author[ ]{Qiming Bao \quad Sherry J.~H. Feng}
\affil[ ]{\normalfont\small Custodian AI}
\date{}
...
\begin{tikzpicture}[
  ...
  box/.style={draw=cline,rounded corners=2pt,align=center,inner sep=4pt,minimum height=8mm,fill=cpaper},
  op/.style={draw=caccent,rounded corners=2pt,align=center,inner sep=4pt,minimum height=8mm,fill=caccent!8},
  res/.style={draw=cclay,rounded corners=2pt,align=center,inner sep=4pt,minimum height=8mm,fill=cclay!8},
  ar/.style={->,thick}
]
...
\textbf{Benchmarks} (7; 250 docs each, 1{,}750 total). ASQ-PHI (English clinical queries), MEDDOCAN (Spanish clinical)~\citep{marimon2019meddocan}, MultiCoNER~v2 (multilingual NER)~\citep{malmasi2022multiconer,fetahu2023multiconer}, and PII-Masking-300k~\citep{ai4privacy2023} in English, Dutch, French, German. Together: 7 languages, clinical and general-PII text, free-form and structured (JSON) formats. The transform under test is Custodian Guardian Layer \tp{transform} mode (top-1 surrogate, \tp{pii\_entities=ALL}).
...
(1)~\emph{Malformed/truncated surrogates} (largest cause): \oldv{\tp{Chicago}}\arr\newv{\tp{Illino}}, \oldv{\tp{El~Paso}}\arr\newv{\tp{El}}, \oldv{\tp{Ciudad de la Habana}}\arr\newv{\tp{Cuidad de la Havana}}. The fragment no longer matches the lexical pattern detectors learned for real names/places; this is why span-level loss is highest on MEDDOCAN (6.9\%; Spanish, dense, identifier-heavy) and lowest on clean English ASQ-PHI (2.6\%).
...
\newcommand{\panel}[3]{\fcolorbox{cline}{white}{\parbox{0.96\textwidth}{%
\textbf{#1}\hfill{\scriptsize\color{cmuted}#2}\\[2pt]\ttfamily\scriptsize #3}}\\[4pt]}
\setlength{\fboxsep}{1pt}%
...
\panel{Microsoft Presidio}{tp 3 $\cdot$ fp 1 $\cdot$ missed 1}{Evaluation of long-term outcomes for bypass surgery in patients over \hy{60}, referencing Mr.\ \hg{John T.}, operated at \hr{Saint. Vxxxxxxxx} on \hg{April 25th, 2018} (ID: \hg{987654321})?}
\panel{OBI \texttt{deid\_roberta}}{tp 4 (fragmented) $\cdot$ fp 1 $\cdot$ missed 0}{Evaluation of long-term outcomes for bypass surgery in patients over \hy{60}, referencing Mr.\ \hg{John}\,\hg{T}., operated at \hg{Saint}.\,\hg{Vxxxxxxxx} on \hg{April 25th,}\,\hg{2018} (ID: \hg{9876543}\,\hg{21})?}
\panel{OpenAI GPT-5}{tp 4 $\cdot$ fp 0 $\cdot$ missed 0}{Evaluation of long-term outcomes for bypass surgery in patients over 60, referencing \hg{Mr.\ John T.}, operated at \hg{Saint. Vxxxxxxxx} on \hg{April 25th, 2018} (\hg{ID: 987654321})?}
\caption{Ranking proof: one transformed document (\texttt{asq\_00001}), several detectors overlaid. \hg{green}~=~correctly caught PHI, \hy{yellow}~=~false positive, \hr{red}~=~missed PHI (leakage). The strong LLM (GPT-5) tags all four surrogates cleanly; Presidio misses the \emph{x}-masked location surrogate \tp{Saint. Vxxxxxxxx} (§\ref{sec:error} mode~3) and false-fires on the age \tp{60}; the fine-tuned tagger finds the entities but over-fragments their boundaries. The gold surrogate values themselves (e.g.\ \tp{John T.}) are the transform's output, not the original PHI.}
\label{fig:rankproof}
\end{figure*}
