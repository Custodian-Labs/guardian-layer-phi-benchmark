...
\bottomrule
\end{tabular}
\caption{C1 redaction floor: masked-span recall (\%) after replacing each masked value with \tp{*****}, across detector families. Rule/fine-tuned detectors collapse to $\approx$0; open LLMs partly recover the redaction cue (context-dependent). All floors sit well below the surrogate recall in Table~\ref{tab:c2}. LLM and OBI cells use a 120-document subsample per benchmark; Presidio is the full corpus.}\label{tab:c1}
\end{table}
...
\bottomrule
\end{tabular}
\caption{C2 open-surrogate (Faker) retention: masked-span recall (\%) on the same masked spans, across detector families. Every family stays high (means 84--95\%), so detectability preservation is generic to well-formed substitution rather than tool- or detector-specific. Subsampling as in Table~\ref{tab:c1}.}\label{tab:c2}
\end{table}
