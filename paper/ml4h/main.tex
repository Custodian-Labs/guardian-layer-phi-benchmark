...
\textbf{Recall retention on masked spans (overlap).} Restricting to masked spans removes that dilution. When the transform masks a span, detectors still find the surrogate 93--100\% of the time (Table~\ref{tab:ret}); the $\sim$3-point exact-boundary drop is a length-jitter artifact that vanishes under overlap matching. (Two floor detectors, Llama~3.1-8B and OBI, are omitted from Table~\ref{tab:ret}: their original recall is too low for a stable ratio.)
...
\textbf{Where the loss lands (per benchmark).} Table~\ref{tab:perbench} breaks whole-document $\Delta$F1 out by benchmark: almost all loss concentrates on \textbf{ASQ-PHI} ($-0.09$ to $-0.18$; short, sparse-PHI adversarial queries where one substitution dominates the score), while the other six are essentially flat ($|\Delta\text{F1}|\le0.06$). The effect tracks \emph{surrogate-generation difficulty}, not detector family---made precise in \S\ref{sec:error}.
...
\textbf{Coverage, reported separately.} All results above are on the spans the transform masks. Coverage---how much true PHI it detects and replaces---is a distinct, detector-side axis: it masks \textbf{80.9\%} of gold PHI on ASQ-PHI, \textbf{48.3\%} on MEDDOCAN, and $\approx$26\% on general-domain corpora whose annotated entities (encyclopedic names, generic places) fall outside its scope (per-type breakdown and a detection-vs-replacement diagnosis---$\approx$77\% of missed clinical identifiers were flagged but not substituted---in Appendix~\ref{app:coverage}). Decoupling matters: an under-masked span is a \emph{detection/replacement} miss, a masked-but-missed surrogate a \emph{generation} defect (\S\ref{sec:error}); conflating them on whole-document F1 would let a high-coverage/low-quality transform and a low-coverage/high-quality one look identical. We report coverage as context and reserve the utility claim for masked spans.

\textbf{Matching modes.} Retention uses \emph{overlap} matching unless noted; exact matching (Table~\ref{tab:ret}, left) also requires identical boundaries, so the $\sim$3-point exact$-$overlap gap is length jitter (``Anna~S.''$\rightarrow$``Maria~S.'' shifts the end offset), not missed PHI.
...
\section{Discussion}
Structure-preserving substitution does not hide well-formed PHI from downstream detection: the effect is equivalence-bounded within $\pm2$ points and ranking-preserving across a heterogeneous 11-detector panel, so a transform of this quality can be inserted ahead of detection/analytics pipelines built for real clinical text without materially degrading detection. C2 shows this is a property of well-formed substitution, not of one vendor---where the commercial generator trails, a cleaner one closes the gap, locating the residual in generation quality. The paired masked-span design and TOST margin are not specific to one vendor or language---a reusable template for auditing any structure-preserving privacy transform.
...
\section{Limitations}
Coverage is reported but not the focus; a transform can preserve utility on what it masks while under-masking (independent axes). 250~docs/benchmark bounds per-benchmark power, though the pooled masked-span $N$ is large. C1/C2 span four detectors across families (Presidio full-corpus; OBI, Qwen-9B, Gemma-31B on a 120-doc/benchmark subsample); a full 11-detector extension is straightforward future work. LLM detectors are prompt-sensitive~\citep{gendron2024abstract,bao2024robustness}; we fix one prompt per model and release it. Finally, all benchmarks are public or synthetic; validating on real-EHR corpora (n2c2, MIMIC-IV-Note, CARMEN-I) awaits the relevant data-use agreements.
